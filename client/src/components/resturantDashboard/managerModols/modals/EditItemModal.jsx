  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-2 sm:p-4">
        <div className="bg-white w-full max-w-2xl max-h-[95vh] overflow-y-auto rounded-lg shadow-lg">
          {/* Header */}
          <div className="flex justify-between px-4 sm:px-6 py-4 border-b border-gray-300 items-center sticky top-0 bg-white z-10">
            ...
            <button
              onClick={handleClose}
              className="text-gray-600 hover:text-red-600 text-xl sm:text-2xl transition"
            >
              ⊗
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6">
            {/* Item Image Section */}
            <div>
              <h3 className="text-base sm:text-lg font-semibold text-gray-700 mb-4 pb-2 border-b border-gray-200">
                Update Item Image
              </h3>
              ...
              <div className="flex flex-col sm:flex-row sm:items-end gap-4">
                <label
                  htmlFor="image"
                  className="px-5 py-2 w-fit bg-(--color-secondary) text-white rounded-md hover:bg-(--color-secondary-hover) cursor-pointer transition flex items-center gap-2 text-sm sm:text-base"
                >
                  Add Image
                </label>

                <div className="flex flex-col text-xs sm:text-sm text-gray-600">
                  <span>(Upto 5 Images Allowed)</span>
                  <span>(Max Size: 1MB each)</span>
                </div>
              ...
              {imagePreviews.length !== 0 && (
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                  {imagePreviews.map((itemImg, idx) => (
                    <div
                      className="border rounded-md aspect-square overflow-hidden"
                      key={idx}
                    >
                      <img
                        src={itemImg}
            ...
            {/* Form Actions */}
            <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-gray-300">
              <button
                type="button"
                onClick={handleClose}
                disabled={loading}
                className="w-full sm:w-auto px-6 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 transition disabled:opacity-50"
              >
                Cancel
              </button>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full sm:w-auto px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
